...
# Preprocessing and displaying functions
...
# Calculate effectiveness for Nash equilibrium
def calculate_effectiveness(method, pixel_value, accuracy):
    if pixel_value:  # Pixel is foreground
        return accuracy  # Effectiveness is the accuracy itself
    else:
        return 1 - accuracy  # Effectiveness for background
...
def find_nash_equilibrium(height, width, pixel_strategies, method_accuracies):
    ...
    for y in range(height):
        for x in range(width):
            # Calculate the effectiveness of each strategy at the current pixel
            strategy_effectiveness = {
                method: calculate_effectiveness(method, pixel_strategies[method][y, x], method_accuracies[method])
                for method in method_accuracies
            }
            
            # Find the strategy with the maximum effectiveness
            nash_strategy = max(strategy_effectiveness, key=strategy_effectiveness.get)
            
            # Apply Nash strategy to the pixel
            final_segmentation[y, x] = 255 if pixel_strategies[nash_strategy][y, x] else 0
